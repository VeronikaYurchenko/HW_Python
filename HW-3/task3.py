...
work_text = "Python is an easy to learn, powerful programming language.\
...
from the Python web site, https://www.python.org/, and may be freely distributed. \
The same site also contains distributions of and pointers to many free third party Python modules, programs and tools,\
 and additional documentation. The Python interpreter is easily extended with new functions\
and data types implemented in C or C++ (or other languages callable from C). \
...
punct = ".,!?;:-[]{}()="
sorted_text = [w.rstrip(punct).lower() for w in work_text.split() if len(w.rstrip(punct)) > 1]
print(*sorted(set(sorted_text), key=sorted_text.count, reverse=True)[:FREQUENT_WORDS], sep='\n')
